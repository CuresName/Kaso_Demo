...
function routeForMegaAction(label) {
  ...
  if (/機票|飯店|旅遊預算/.test(label)) return "/travel";
  if (/保險|保障|理賠|保單/.test(label)) return "/insurance";
  if (/訂閱|音樂|影音/.test(label)) return "/subscriptions";
  if (/記帳|支出|目標/.test(label)) return "/ledger";
...
function closeOpenOverlays() {
  hideMegaMenu();
  const serviceSheet = $("#serviceSheet");
